...
export default function Detalhes(props) {
  ...
  const angulo = -30;
  const [jaAnimouSininho, setJaAnimouSininho] = useState(false);
  ...
  const estiloAnimadoSininho = useAnimatedStyle(() => {
    return {
      transform: [
        {
        ...
        />
          ...
          >
            <Text style={styles.botaoTexto}>Notificar consulta</Text>
              <Animated.View style={[styles.icone, estiloAnimadoSininho]}>
              <Icon 
                name={jaAnimouSininho ? 'notifications' : 'notifications-none'} 
                size={20} 
                color="#FFF"
              />
            </Animated.View>
